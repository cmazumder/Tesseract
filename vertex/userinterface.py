...
class UI(apiServices):

    def __init__(self):
        apiServices.__init__(self)
        try:
            self.build_id = self.get_buildId_from_buildTypeId(
                buildTypeId=teamcity_download_setting["ui"]["buildTypeID"])
            self.folder = application_structure["ui"]["folder_name"]
    ...
    def set_vertex_ui_properties(self):
        try:
            anchor_text = teamcity_download_setting["ui"]["anchor"]
