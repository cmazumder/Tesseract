...
class Shell(apiServices):
        ...
        try:
            self.build_id = \
                self.get_buildId_from_buildTypeId(
                    buildTypeId=teamcity_download_setting["shell"]["buildTypeID"])
            self.folder = application_structure["shell"]["folder_name"]
    ...
    def set_vertex_shell_properties(self):
        try:
            anchor_text = teamcity_download_setting["shell"]["anchor"]
    ...
    def download_vertex_shell(self):
