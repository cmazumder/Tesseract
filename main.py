...
def artifact_download_with_timer(test_mode):
    # Folder.delete_folder_contents(deployment_env_paths["path_download_root"])
    file_name = "BuildDeploymentInfo_" + datetime.now().strftime("%Y-%m-%dT%H%M%S") + ".txt"
    file_version_txt = Folder.build_path(deployment_env_paths["path_download_root"], file_name)
    start_s = time()
    ...
    print "{}\n{}".format('#' * 45, '  *' * 15)
    print "Artifact download time: {}".format(get_readable_epoc_time(time_download))
    print "Artifact replacement time: {}".format(get_readable_epoc_time(time_replace))
    print "Database re-create time: {}".format(get_readable_epoc_time(time_db))
    print "{}\n{}".format("  *" * 15, "#" * 45)

    File.append_text_to_file(file_version_txt, "\t\tElapsed time (hh:mm:ss)\n")
    File.append_text_to_file(file_version_txt, "\tDownload build -----> ", get_readable_epoc_time(time_download))
    File.append_text_to_file(file_version_txt, "\tReplace build  -----> ", get_readable_epoc_time(time_replace))
    File.append_text_to_file(file_version_txt, "\tDatabase recreate --> ", get_readable_epoc_time(time_db))
    end_s = time()
    time_s = end_s - start_s
    File.append_text_to_file(file_version_txt, "\tTotal execution ----> ", get_readable_epoc_time(time_s))
    File.append_text_to_file(file_version_txt, spacer, "\n", spacer)

    File.append_text_to_file(file_version_txt, "\t\tTime (local time)\n")
...
def get_readable_epoc_time(float_time):
