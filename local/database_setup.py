...
class DatabaseSetup:
    ...
    def delete_existing_database(database_connection, env_setting):
        db_to_delete = get_dict_value(env_setting, ["db_property", "db_to_delete"])  # type: list
    ...
    def setup_sql_script(sql_path, env_setting):
        find_text = []
        replace_text = []
        if str(get_dict_value(env_setting, ["db_property", "db_script_mod", "find_replace", "mod"])).lower() == 'yes':
            find_text = get_dict_value(env_setting, ["db_property", "db_script_mod", "find_replace", "find_text"])
            replace_text = get_dict_value(env_setting, ["db_property", "db_script_mod", "find_replace", "replace_text"])

        if str(get_dict_value(env_setting, ["db_property", "db_script_mod", "db_size", "mod"])).lower() == 'yes':
            find_text.append(get_dict_value(env_setting, ["db_property", "db_script_mod", "db_size", "original_size"]))
            replace_text.append(get_dict_value(env_setting, ["db_property", "db_script_mod", "db_size", "new_size"]))
